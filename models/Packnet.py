    ...
    def prune(self, t,model, prune_quantile):
        ...
        for name, param_layer in model.named_parameters():
            if 'bias' not in name:
                ...
                prev_mask = torch.zeros(param_layer.size(), dtype=torch.bool, requires_grad=False).cuda()
        ...
        with torch.no_grad():
            for name, param_layer in model.named_parameters():
                if 'bias' not in name:
                    ...
                    prev_mask = torch.zeros(param_layer.size(), dtype=torch.bool, requires_grad=False).cuda()  # p
                    for task in self.masks:
                        if name in task:
                            prev_mask |= task[name]
